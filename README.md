...
고난도 수열추리 연습 웹앱입니다. 6가지 복합 규칙으로 생성한 창작 문제 2,249개에서 10문항 또는 20문항 실전 모드와 한 문제씩 즉시 해설을 보는 학습 모드로 풀 수 있습니다. 실전 모드는 마지막에 전체 해설을 보여줍니다. 정답과 풀이 시간에 따른 점수는 문항 수별 리더보드에 등록할 수 있습니다. 풀이 화면 아래에는 그림판과 메모장이 있고, 문제를 넘기면 둘 다 초기화됩니다. 답안은 선택지를 마우스로 누른 뒤 정답 확인 버튼으로 제출합니다.
